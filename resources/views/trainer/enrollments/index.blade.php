...
<div x-data="{
    ...
    <div class="mx-auto max-w-7xl space-y-8">
        ...
        <div class="rounded-3xl bg-gradient-to-r from-slate-950 via-green-950 to-slate-900 p-6 sm:p-10 text-white shadow-xl border border-green-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <div class="shrink-0 bg-white/10 backdrop-blur rounded-2xl p-5 border border-white/10 text-center min-w-[150px]">
                <span class="text-xs font-bold text-green-300 uppercase tracking-wider">Total Learners</span>
                <p class="text-3xl font-black text-green-400 mt-0.5">{{ $enrollments->total() }}</p>
                        ...
                        @forelse($enrollments as $enr)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="py-4 px-3">
                                    ...
                                    <span class="text-[11px] text-green-700 font-semibold">{{ ucfirst($enr->course_type) }}</span>
                                </td>
                                <td class="py-4 px-3 font-bold">
                                    @if(!is_null($enr->score))
                                        <span class="{{ $enr->score >= 80 ? 'text-green-700' : 'text-rose-600' }}">
                                ...
                                <td class="py-4 px-3">
                                    <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-bold 
                                        {{ $enr->status === 'completed' ? 'bg-green-100 text-green-800 border border-green-300' : ($enr->status === 'in_progress' ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-slate-100 text-slate-700') }}">
                                ...
                                <td class="py-4 px-3">
                                    @if($enr->certificate_issued)
                                        <div class="space-y-0.5">
                                            <span class="inline-flex items-center gap-1 rounded-full bg-green-600 text-white px-2.5 py-0.5 text-[10px] font-black">
                                                🎓 {{ $enr->certificate_no }}
                                            </span>
                                            <a href="{{ route('trainer.certificates.preview', $enr->enrollment_id) }}" target="_blank" class="block text-[10px] font-bold text-green-700 hover:underline">
                                ...
                                <td class="py-4 px-3 text-right">
                                    <div class="flex items-center justify-end gap-1.5 flex-wrap">
                                        ...
                                        <a href="{{ route('trainer.enrollments.assessment', $enr->enrollment_id) }}" 
                                           class="px-2.5 py-1.5 rounded-xl bg-slate-900 hover:bg-green-600 text-white text-[11px] font-bold transition-colors"
                                        ...
                                        @if($enr->status !== 'completed')
                                            <form action="{{ route('trainer.enrollments.complete', $enr->enrollment_id) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit" onclick="return confirm('Mark this enrollment as Completed and award verified skills to the jobseeker?')"
                                                        class="px-2.5 py-1.5 rounded-xl bg-green-600 hover:bg-green-500 text-white text-[11px] font-black transition-colors"
                                        ...
                                        @if(!$enr->certificate_issued)
                                            <form action="{{ route('trainer.enrollments.certificate', $enr->enrollment_id) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit" onclick="return confirm('Generate and issue official skills certificate?')"
                                                        class="px-2.5 py-1.5 rounded-xl bg-green-600 hover:bg-green-500 text-white text-[11px] font-black shadow-md shadow-green-600/20"
    ...
    <div x-show="statusModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div @click.away="statusModal = false" class="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-6">
            ...
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <div>
                    <span class="text-xs font-bold text-green-700 uppercase tracking-wider">Update Enrollment Status</span>
