...
<div x-data="{ 
        ...
        <div class="rounded-2xl bg-white p-4 border border-slate-200 shadow-sm flex items-center justify-between gap-4">
            <form action="{{ route('trainer.skills.enrollment') }}" method="GET" class="w-full flex items-center gap-3">
                <div class="relative w-full sm:w-96">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search candidate name or email..." class="w-full rounded-xl border border-slate-200 pl-9 pr-4 py-2 text-xs font-medium text-slate-800 placeholder-slate-400 focus:border-green-500 outline-none">
                </div>
                                ...
                                @forelse($jobseekers as $js)
                                    <tr class="hover:bg-slate-50/50">
                                        ...
                                        <td class="py-3 px-3 text-right">
                                            <button @click="selectedJobseeker = { id: {{ $js->jobseeker_id }}, name: '{{ addslashes($js->first_name . ' ' . $js->last_name) }}', email: '{{ addslashes($js->user_email) }}' }; enrollModal = true;" type="button" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-green-50 hover:bg-green-100 text-green-800 text-xs font-extrabold transition-colors">
                            ...
                            <div class="p-3.5 rounded-2xl border border-slate-100 bg-slate-50 space-y-2 text-xs">
                                <div class="flex items-start justify-between gap-2">
                                    <div>
                                        ...
                                        <span class="text-[11px] text-green-700 font-semibold">{{ $renr->course_title }}</span>
                                    </div>
                                    <span class="rounded-full px-2 py-0.5 text-[9px] font-black uppercase {{ $renr->status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                                        {{ $renr->status }}
                                    </span>
                                </div>
    ...
    <div x-show="enrollModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="enrollModal" x-transition class="relative inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-green-100">
                <form action="{{ route('trainer.skills.enrollment.store') }}" method="POST">
                    ...
                    <div class="bg-gradient-to-r from-slate-950 via-green-950 to-slate-900 px-6 py-5 text-white flex items-center justify-between border-b border-green-500/20">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-green-300/80">Target specific skills for candidate development.</p>
                            </div>
                        </div>
                    ...
                    <div class="p-6 space-y-4">
                        <div class="rounded-2xl bg-green-50/80 border border-green-100 p-3.5 space-y-1">
                            <span class="text-[10px] uppercase font-bold text-green-700 tracking-wider">Candidate</span>
                        ...
                        <div>
                            ...
                            <select name="training_id" x-model="selectedCourseId" @change="onCourseChange()" required class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-medium text-slate-800 focus:border-green-500 outline-none">
                        ...
                        <div>
                            ...
                            <input type="text" name="enrolled_skills" x-model="targetSkills" required placeholder="e.g. Electrical Safety, Circuit Diagnostics, Wiring" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-medium text-slate-800 focus:border-green-500 outline-none">
