...
@section('title', 'Trainer Notifications - Skills Trainer')
    ...
    <div class="mx-auto max-w-4xl space-y-8">
        ...
        <div class="rounded-3xl bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-900 p-6 sm:p-10 text-white shadow-xl border border-emerald-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="space-y-1">
                ...
                <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Trainer Notifications</h1>
                <p class="text-xs text-slate-300">Live operational alerts for trainee progress, assessments, and certifications.</p>
            </div>
            
            <div class="flex items-center gap-3">
                @if(isset($unreadCount) && $unreadCount > 0)
                    <form action="{{ route('trainer.notifications.read_all') }}" method="POST">
                        @csrf
                        <button type="submit" 
            ...
            <div class="divide-y divide-slate-100">
                @forelse($notifications as $notif)
                    <div class="py-4 flex items-start justify-between gap-4 {{ $notif->is_read ? 'opacity-75' : 'bg-emerald-50/30 rounded-2xl p-4 my-1 border border-emerald-100/60' }}">
                        ...
                        @if(!$notif->is_read)
                            <form action="{{ route('trainer.notifications.read', $notif->notification_id) }}" method="POST" class="shrink-0">
                                @csrf
                ...
                @empty
                    <div class="text-center py-12 space-y-3">
                        ...
                        <p class="text-xs text-slate-500 max-w-sm mx-auto">You're completely up to date. Trainee activities, submissions, and course updates will appear here as they occur.</p>
                    </div>
                @endforelse
            </div>
