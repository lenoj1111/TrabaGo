...
@section('title', 'JPO Notifications - Placement Officer')
        ...
        <div class="rounded-3xl bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-900 p-6 sm:p-10 text-white shadow-xl border border-emerald-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="space-y-1">
                <div class="inline-flex items-center gap-2 rounded-full bg-emerald-400/20 px-3 py-1 text-xs font-bold text-emerald-300 border border-emerald-400/30">
                    ...
                    Alerts & Pipeline Activity
                </div>
                <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Placement Officer Notifications</h1>
                <p class="text-xs text-slate-300">Live operational alerts for applicant referrals, employer accreditation reviews, and placement report submissions.</p>
            </div>
            
            <div class="flex items-center gap-3">
                @if(isset($unreadCount) && $unreadCount > 0)
                    <form action="{{ route('jpo.notifications.read_all') }}" method="POST">
                        @csrf
                        <button type="submit" 
            ...
            <div class="divide-y divide-slate-100">
                @forelse($notifications as $notif)
                    <div class="py-4 flex items-start justify-between gap-4 {{ $notif->is_read ? 'opacity-75' : 'bg-emerald-50/30 rounded-2xl p-4 my-1 border border-emerald-100/60' }}">
                        ...
                        @if(!$notif->is_read)
                            <form action="{{ route('jpo.notifications.read', $notif->notification_id) }}" method="POST" class="shrink-0">
                                @csrf
                ...
                @empty
                    <div class="text-center py-12 space-y-3">
                        ...
                        <p class="text-xs text-slate-500 max-w-sm mx-auto">You're completely up to date. Candidate referral requests, employer accreditation packets, and placement reports will appear here.</p>
                    </div>
                @endforelse
            </div>
