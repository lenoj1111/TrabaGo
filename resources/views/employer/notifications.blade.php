...
@section('title', 'Employer Notifications - DMDP Corporate Partner')
    ...
    <div class="mx-auto max-w-4xl space-y-8">
        ...
        <div class="rounded-3xl bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-900 p-6 sm:p-10 text-white shadow-xl border border-emerald-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="space-y-1">
                ...
                <p class="text-xs text-slate-300">Live operational alerts for job applications, referred candidates, and accreditation reviews.</p>
            </div>
            
            <div class="flex items-center gap-3">
                @if(isset($unreadCount) && $unreadCount > 0)
                    <form action="{{ route('employer.notifications.read_all') }}" method="POST">
                        @csrf
                        <button type="submit" 
            ...
            <div class="divide-y divide-slate-100">
                ...
                @empty
                    <div class="text-center py-12 space-y-3">
                        ...
                        <p class="text-xs text-slate-500 max-w-sm mx-auto">You're completely up to date. Candidate referrals, job application updates, and accreditation status changes will appear here.</p>
                    </div>
                @endforelse
            </div>
