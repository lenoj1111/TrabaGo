        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-6 xl:gap-8">
                    <a href="{{ route('trainer.dashboard') }}" class="flex items-center gap-2.5 group shrink-0">
                        <div class="h-10 w-10 rounded-2xl bg-gradient-to-tr from-emerald-950 via-emerald-800 to-teal-600 flex items-center justify-center text-white font-black text-xl shadow-md shadow-emerald-900/30 group-hover:scale-105 transition-transform">
                            🎓
                        </div>
                        <div class="flex flex-col">
                            ...
                            <span class="text-[10px] font-bold text-emerald-700 tracking-widest uppercase leading-none mt-0.5">Skills Trainer Portal</span>
                        </div>
                    </a>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('trainer.notifications') }}" 
                       class="relative p-2 rounded-xl text-slate-500 hover:text-emerald-700 hover:bg-emerald-50 transition-colors {{ request()->routeIs('trainer.notifications*') ? 'bg-emerald-50 text-emerald-800' : '' }}"
                       title="Notifications">
                    ...
                    <div class="relative" @click.away="profileOpen = false">
                        <button @click="profileOpen = !profileOpen" type="button" class="flex items-center gap-2.5 p-1 rounded-xl hover:bg-emerald-50 transition-colors text-left focus:outline-none">
                            ...
                            <div class="hidden lg:flex flex-col">
                                ...
                                <span class="text-[10px] font-semibold text-emerald-700 leading-none mt-0.5">Skills Trainer</span>
                            </div>
                        ...
                        <div x-show="profileOpen" x-cloak class="absolute right-0 mt-2 w-60 rounded-2xl bg-white p-2 shadow-2xl border border-emerald-100 focus:outline-none z-50">
                            ...
                            <a href="{{ route('trainer.profile') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 transition-colors {{ request()->routeIs('trainer.profile*') ? 'bg-emerald-50 text-emerald-800' : '' }}">
                                <span>👤</span> Edit Profile
                            </a>

                            <a href="{{ route('trainer.notifications') }}" class="flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 transition-colors {{ request()->routeIs('trainer.notifications*') ? 'bg-emerald-50 text-emerald-800' : '' }}">
                                <span class="flex items-center gap-2.5">
                                    <span>🔔</span> Notifications
                                </span>
                                @if($trainerUnreadCount > 0)
                                    <span class="px-1.5 py-0.5 text-[10px] bg-emerald-600 text-white rounded-full font-black">{{ $trainerUnreadCount }}</span>
                                @endif
                            </a>

                            <a href="{{ route('trainer.courses') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 transition-colors">
