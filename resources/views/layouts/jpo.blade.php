        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-6 xl:gap-8">
                    <a href="{{ route('jpo.dashboard') }}" class="flex items-center gap-2.5 group shrink-0">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-[10px] font-bold text-emerald-700 tracking-widest uppercase leading-none mt-0.5">Job Placement Officer</span>
                        </div>
                    </a>
                    ...
                    <nav class="hidden md:flex items-center gap-1.5">
                        <a href="{{ route('jpo.dashboard') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all {{ request()->routeIs('jpo.dashboard*') ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200' : 'text-slate-600 hover:text-emerald-700 hover:bg-slate-50' }}">
                            Dashboard
                        </a>
                        <a href="{{ route('jpo.evaluations.jobseekers') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all {{ request()->routeIs('jpo.evaluations.jobseekers*') ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200' : 'text-slate-600 hover:text-emerald-700 hover:bg-slate-50' }}">
                            Evaluate Jobseekers
                        </a>
                        <a href="{{ route('jpo.evaluations.accreditations') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all {{ request()->routeIs('jpo.evaluations.accreditations*') ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200' : 'text-slate-600 hover:text-emerald-700 hover:bg-slate-50' }}">
                            Accreditation Papers
                        </a>
                        <a href="{{ route('jpo.evaluations.placement-reports') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all {{ request()->routeIs('jpo.evaluations.placement-reports*') ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200' : 'text-slate-600 hover:text-emerald-700 hover:bg-slate-50' }}">
                            Placement Reports
                        </a>
                    </nav>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('jpo.notifications') }}" 
                       class="relative p-2 rounded-xl text-slate-500 hover:text-emerald-700 hover:bg-emerald-50 transition-colors {{ request()->routeIs('jpo.notifications*') ? 'bg-emerald-50 text-emerald-800' : '' }}"
                       title="Notifications">
                    ...
                    <div class="relative" @click.away="profileOpen = false">
                        <button @click="profileOpen = !profileOpen" type="button" class="flex items-center gap-2.5 p-1 rounded-xl hover:bg-emerald-50 transition-colors text-left focus:outline-none">
                            ...
                            <div class="hidden lg:flex flex-col">
                                ...
                                <span class="text-[10px] font-semibold text-emerald-700 leading-none mt-0.5">Placement Officer</span>
                            </div>
                        ...
                        <div x-show="profileOpen" x-cloak class="absolute right-0 mt-2 w-60 rounded-2xl bg-white p-2 shadow-2xl border border-emerald-100 focus:outline-none z-50">
                            ...
                            <a href="{{ route('jpo.notifications') }}" class="flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 transition-colors {{ request()->routeIs('jpo.notifications*') ? 'bg-emerald-50 text-emerald-800' : '' }}">
                                ...
                                @if($jpoUnreadCount > 0)
                                    <span class="px-1.5 py-0.5 text-[10px] bg-emerald-600 text-white rounded-full font-black">{{ $jpoUnreadCount }}</span>
                                @endif
                            </a>

                            <a href="{{ route('jpo.evaluations.placement-reports') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 transition-colors">
            ...
            <div x-show="mobileOpen" x-cloak class="md:hidden border-t border-emerald-100 py-3 space-y-1">
                <a href="{{ route('jpo.dashboard') }}" class="block px-3.5 py-2.5 rounded-xl text-xs font-bold {{ request()->routeIs('jpo.dashboard*') ? 'bg-emerald-50 text-emerald-800' : 'text-slate-600' }}">Dashboard</a>
                <a href="{{ route('jpo.evaluations.jobseekers') }}" class="block px-3.5 py-2.5 rounded-xl text-xs font-bold {{ request()->routeIs('jpo.evaluations.jobseekers*') ? 'bg-emerald-50 text-emerald-800' : 'text-slate-600' }}">Evaluate Jobseekers</a>
                <a href="{{ route('jpo.evaluations.accreditations') }}" class="block px-3.5 py-2.5 rounded-xl text-xs font-bold {{ request()->routeIs('jpo.evaluations.accreditations*') ? 'bg-emerald-50 text-emerald-800' : 'text-slate-600' }}">Accreditation Papers</a>
                <a href="{{ route('jpo.evaluations.placement-reports') }}" class="block px-3.5 py-2.5 rounded-xl text-xs font-bold {{ request()->routeIs('jpo.evaluations.placement-reports*') ? 'bg-emerald-50 text-emerald-800' : 'text-slate-600' }}">Placement Reports</a>
                <a href="{{ route('jpo.notifications') }}" class="flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold {{ request()->routeIs('jpo.notifications*') ? 'bg-emerald-50 text-emerald-800' : 'text-slate-600' }}">
