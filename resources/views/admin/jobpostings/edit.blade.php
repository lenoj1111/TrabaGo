...
<div class="container-fluid">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.job-postings.update', $jobPosting->job_id) }}">
                @csrf
                @method('PUT')
                            ...
                            <select name="employer_id" class="form-select @error('employer_id') is-invalid @enderror" required>
                                ...
                                @foreach($employers as $employer)
                                    <option value="{{ $employer->employer_id }}" 
                                        {{ old('employer_id', $jobPosting->employer_id) == $employer->employer_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="vacancy_count" class="form-control @error('vacancy_count') is-invalid @enderror" 
                                   value="{{ old('vacancy_count', $jobPosting->vacancy_count) }}" min="1" required>
                            @error('vacancy_count')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" 
                                   value="{{ old('title', $jobPosting->title) }}" placeholder="e.g., Senior Software Developer" required>
                            @error('title')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                                      rows="5" placeholder="Describe the job responsibilities, benefits, etc." required>{{ old('description', $jobPosting->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <textarea name="qualifications" class="form-control @error('qualifications') is-invalid @enderror" 
                                      rows="4" placeholder="List the qualifications, skills, and experience required.">{{ old('qualifications', $jobPosting->qualifications) }}</textarea>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" name="valid_until" class="form-control @error('valid_until') is-invalid @enderror" 
                                   value="{{ old('valid_until', $jobPosting->valid_until) }}" min="{{ date('Y-m-d', strtotime('+1 day')) }}" required>
                            @error('valid_until')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input type="checkbox" name="accepts_disability" class="form-check-input" 
                                       value="1" id="acceptsDisability" {{ old('accepts_disability', $jobPosting->accepts_disability) ? 'checked' : '' }}>
                                <label class="form-check-label" for="acceptsDisability">
                                    This job accepts applicants with disabilities
                                </label>
                            </div>
                            <div id="disabilityTypeContainer" style="{{ old('accepts_disability', $jobPosting->accepts_disability) ? '' : 'display: none;' }}">
                                <input type="text" name="disability_type" class="form-control mt-2" 
                                       ...
                                       value="{{ old('disability_type', $jobPosting->disability_type) }}">
                            </div>
                        </div>
                    </div>
                </div>
