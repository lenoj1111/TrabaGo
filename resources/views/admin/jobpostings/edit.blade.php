...
<div class="min-h-screen bg-slate-50/80 px-4 py-8 sm:px-6 lg:px-8" x-data="{ acceptsDisability: {{ old('accepts_disability', $jobPosting->accepts_disability) ? 'true' : 'false' }} }">
    <div class="mx-auto max-w-4xl space-y-8">
        ...
        <div class="flex items-center justify-between">
            ...
            <span class="inline-flex items-center px-3 py-1 rounded-xl text-xs font-bold border {{ $jobPosting->status === 'approved' ? 'bg-green-50 text-green-800 border-green-200' : 'bg-amber-50 text-amber-800 border-amber-200' }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="sm:col-span-2 space-y-1.5">
                        ...
                        <textarea name="description" rows="5" required placeholder="Describe duties and benefits..."
                                  class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none @error('description') border-rose-400 @enderror">{{ old('description', $jobPosting->description) }}</textarea>
                        @error('description')
                            <p class="text-[11px] font-bold text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:col-span-2 space-y-1.5">
                        ...
                        <textarea name="qualifications" rows="4" placeholder="Qualifications and skills required..."
                                  class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none @error('qualifications') border-rose-400 @enderror">{{ old('qualifications', $jobPosting->qualifications) }}</textarea>
